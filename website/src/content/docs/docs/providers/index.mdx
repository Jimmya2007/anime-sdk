---
title: Providers
description: Overview of the nine live-tested anime and manga providers in ani-sdk.
---

A provider is a site-specific adapter that maps search queries and episode/chapter IDs to direct stream or image URLs. All nine ship with ani-sdk and are verified on every release.

## Available providers

| ID              | Class                   | Domain            | Content Type | Languages     | Subtitles | Method            |
| --------------- | ----------------------- | ----------------- | ------------ | ------------- | --------- | ----------------- |
| `animeparadise` | `AnimeParadiseProvider` | animeparadise.moe | ANIME        | sub           | yes (VTT) | REST API          |
| `allmanga`      | `AllmangaProvider`      | allmanga.to       | ANIME        | sub, dub, raw | no        | GraphQL + AES-CTR |
| `gogoanime`     | `GogoanimeProvider`     | anineko.to        | ANIME        | sub           | no        | HTML scrape + HLS |
| `anikoto`       | `AnikotoProvider`       | anikototv.to      | ANIME        | sub, dub      | yes (VTT) | JSON API + Embed  |
| `megaplay`      | `MegaPlayProvider`      | megaplay.buzz     | ANIME        | sub, dub      | yes (VTT) | AniList + mapping |
| `goyabu`        | `GoyabuProvider`        | goyabu.io         | ANIME        | raw (pt-br)   | no        | batchexecute RPC  |
| `mangadex`      | `MangadexProvider`      | mangadex.org      | MANGA        | sub           | no        | JSON API          |
| `weebcentral`   | `WeebcentralProvider`   | weebcentral.com   | MANGA        | sub           | no        | HTML scrape       |
| `mangapill`     | `MangapillProvider`     | mangapill.com     | MANGA        | sub           | no        | HTML scrape       |
