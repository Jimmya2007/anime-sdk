...
fetch(url, {
  ...
  .then(async json => {
    ...
    // Find the first clock.json source
    const clockSource = parsed.episode.sourceUrls.find(src => src.sourceUrl.startsWith('--'));
    if (!clockSource) {
      console.log("No clock.json source found!");
      return;
    }

    const decodedClockUrl = decodeSourceURL(clockSource.sourceUrl.slice(2));
    console.log("Decoded Clock URL:", decodedClockUrl);

    // Fetch the clock URL
    const clockRes = await fetch(decodedClockUrl, {
      headers: {
        "Referer": "https://allmanga.to",
        "User-Agent": "Mozilla/5.0 (Windows NT 10.0; Win64; x64; rv:109.0) Gecko/20100101 Firefox/121.0"
      }
    ...
    console.log("Clock HTTP Code:", clockRes.status);
    const clockJson = await clockRes.json();
    console.log("Clock JSON response:");
    console.log(JSON.stringify(clockJson, null, 2));
  })
