...
// Episodes — pass 'sub' or 'dub'; falls back to 'sub' if unavailable
const eps = await provider.fetchContentUnits(shows[0].id, 'dub');
// eps[0] => { id: 'shingeki-no-kyojin/1/dub', title: 'Episode 1',
//             number: 1, language: 'dub' }
...
`search()` returns `availableLanguages` on each result so you can check upfront. `fetchContentUnits()` and `resolveStream()` fall back to `'sub'` if the requested language isn't available for a given title.

## Content unit ID format

AllManga unit IDs embed the show ID, episode string, and language:

```
{showId}/{episodeString}/{language}
```

Example: `shingeki-no-kyojin/1/sub`. Do not construct these manually — always use IDs from `fetchContentUnits`.
...
- Does not require `FlareSolverrClient` — allmanga.to is not Cloudflare-protected.
