...
// eps[0] => { id: '/40742', title: 'Episódio 1', number: 1, language: 'sub' }
...
All content on Goyabu is Brazilian Portuguese (raw audio, no subtitles). Content unit IDs report `language: 'sub'` because that's the `ContentLanguage` type's closest match, but the audio is PT-BR only.

The `language` parameter on `fetchContentUnits` and `resolveStream` is ignored.
