...
Returned by `provider.fetchContentUnits()`. Represents a single episode in a unified, language-agnostic list — the caller picks a translation at `resolveStream` time.
...
interface IContentUnit {
  ...
  availableLanguages: ContentLanguage[]; // e.g. ['sub', 'dub']
...
Launches a Node `http.Server`. Returns the server instance so you can call `server.close()`. All requests must be `GET`; all responses are JSON. See [HTTP Server](/docs/http-server/) for route documentation and [Stream Proxy](/docs/proxy/) for the proxy behaviour.
