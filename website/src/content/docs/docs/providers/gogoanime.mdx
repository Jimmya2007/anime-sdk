...
The language parameter to `fetchContentUnits` and `resolveStream` is accepted but ignored. All content units report `language: 'sub'`.
...
3. **Stream resolution** — GET the episode page. Parses `button.nv-server-btn` elements, each of which has a `data-video` attribute containing a stream URL. Quality is inferred from the button label text. URLs starting with `//` or `/` are made absolute.
...
- `resolveStream` returns all available server streams as separate `IVideoPayload` entries. The first one is not necessarily the highest quality — iterate and try in order if playback fails.
