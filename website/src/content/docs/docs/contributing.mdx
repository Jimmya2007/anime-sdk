...
If the site is Cloudflare-protected, it's still welcome — accept `FlareSolverrClient` as an optional constructor argument and throw a clear error when it's absent.
...
Edge cases in `HlsUtils.rewriteManifest`, `FlareSolverrClient` response parsing, extractor HTML fixtures, language inference — these are always welcome and run in CI without a network.
...
| Area                                 | Why                                                                                                                                                                                                                                             |
| ------------------------------------ | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **Downloading / saving media**       | ani-sdk resolves URLs. Feeding them to `ffmpeg` or `yt-dlp` is the consumer's job, not the SDK's.                                                                                                                                               |
...
| **UI components or players**         | ani-sdk is a headless library. Rendering is out of scope.                                                                                                                                                                                       |
| **Login-gated or paywall sites**     | ani-sdk is for publicly accessible streams. Sites that require accounts are out of scope.                                                                                                                                                       |
...
ani-sdk has three layers:

- **Transport** (`src/transport/`) — `HttpClient`, `DomRegistry`, `HlsUtils`, `FlareSolverrClient`. Rarely needs changes for new providers.
...
If the target site is behind Cloudflare or DDoS-Guard:

```ts
import type { FlareSolverrClient } from '../transport/flaresolverr.js';

export class MyProvider extends BaseProvider {
  constructor(
    http: HttpClient,
    private flare?: FlareSolverrClient,
  ) {
    super(http);
  }
  ...
  public async search(query: string) {
    if (!this.flare) {
      throw new Error('MyProvider requires FlareSolverrClient — this site is CF-protected');
    }
    const res = await this.flare.get(`${this.baseUrl}/search?q=${encodeURIComponent(query)}`);
    // res.text() / res.json() / res.headers / res.url
    const html = res.text();
    // ...
  }
}
...
Check `await flare.isAvailable()` in the test setup to skip gracefully if FlareSolverr isn't running.
...
- [ ] `fetchContentUnits` returns `IContentUnit[]` sorted by `number`
- [ ] `resolveStream` returns `ResolvedMediaStream` — at minimum `{ type: 'video', streams: [...] }`
- [ ] All imports use `.js` extension
- [ ] Exported from `src/index.ts`
- [ ] Live E2E test using `captureStreamScreenshot` — no mocking, 90s timeout
- [ ] CF-protected sites: accepts `FlareSolverrClient` optionally, throws clear error when absent
