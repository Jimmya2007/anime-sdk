...
List episodes for a show. One call returns the unified, language-agnostic list — each episode advertises its translations.
...
Resolve a direct stream URL for an episode. Pick the translation here.
...
When `proxy: true`, both `sourceUrl` and every subtitle `url` are rewritten through `/proxy` automatically.
